...
class AnswerConsolidator:
    ...
    async def consolidate(self, response: WebSearchResponse) -> WebSearchResponse:
        ...
        if self.use_llm:
            ...
            response.answer = await self._consolidate_with_llm(response)
    ...
    async def _consolidate_with_llm(self, response: WebSearchResponse) -> str:
        """Generate a dense, citation-bearing answer using the injected chat model.

        Runs inside LangGraph's event loop and awaits the native LangChain
        ``ainvoke``; the call is traced automatically by any Langfuse callback
        propagated from the parent node.
        """
        if self.llm is None:
            raise ValueError(
                "AnswerConsolidator(use_llm=True) requires an injected chat model (`llm`)."
            )
